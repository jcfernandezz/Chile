...
--16/10/14 jcf Prueba también el separador espacio
...
begin
		return
				...
				--útil para simular facturas reales (segundo paso de certificación). 
				--inicio simulación
...
go
IF (@@Error = 0) PRINT 'Creación exitosa de: fCfdObtieneFolio()'
ELSE PRINT 'Error en la creación de: fCfdObtieneFolio()'
GO
-------------------------------------------------------------------------------------------------------
...
begin
		return 
			--right(@sopnumbe, len(@sopnumbe)-patindex('%'+@separador+'%', @sopnumbe))
				...
				--útil para simular facturas reales (segundo paso de certificación). resta 8128 para igualar el rango de folios disponible
				--inicio simulación
			--convert(varchar(18),
			--	case when @sopnumbe like 'FV%' THEN
			--		CONVERT( INT, 
...
IF (@@Error = 0) PRINT 'Creación exitosa de: fCfdGetSegmento2()'
ELSE PRINT 'Error en la creación de: fCfdGetSegmento2()'
GO
