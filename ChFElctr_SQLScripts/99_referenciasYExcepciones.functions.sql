...
--14/02/19 jcf Agrega parámetros TPODOCREF4 para caso de referencia MIGO
--
return(
			select ROW_NUMBER() OVER(ORDER BY tracking_number) NroLinRef, 
				case when rtrim(upper(Tracking_Number)) like 'OC%' then '801'
					...
					when rtrim(upper(Tracking_Number)) like 'MIGO%' then '52'	--guía de despacho

					when pr.param2 like '%'+left(rtrim(upper(Tracking_Number)), 3)+'%' then	--obtiene código de 3 posiciones: HES, HEM, OTS
						left(rtrim(upper(Tracking_Number)), 3)

					when pr.param1 like '%'+left(rtrim(upper(Tracking_Number)), 2)+'%' then --obtiene código de 2 posiciones: RS, PR, OS, NC
				...
				case 
					when pr.param3 like '%'+left(rtrim(upper(Tracking_Number)), 4)+'%' then
						SUBSTRING(rtrim(Tracking_Number), 5, 40 )
				...
				null				RazonRef
			from sop10107	--
		      cross apply dbo.fLcLvParametros('TPODOCREF2', 'TPODOCREF3', 'TPODOCREF4', 'NA', 'NA', 'NA') pr
			where soptype = 3 
